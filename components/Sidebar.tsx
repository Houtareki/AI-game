...
import { CharacterStats, PlayerStatus, NotebookEntry, Quest, StorySegment, GameState } from '../types';
...
interface SidebarProps {
  ...
  gameState: GameState; // Full state needed for saving
...
const Sidebar: React.FC<SidebarProps> = ({
  segment, notebook, quests, stats, playerStatus, gameState,
  isMobileMenuOpen, setIsMobileMenuOpen,
  onOpenNotebook, onUndo, onLoadGame, onRestart,
      ...
      `}>
        ...
        <div className="p-4 border-t border-gray-800 bg-gray-900 space-y-2 shrink-0">
           ...
           <div className="grid grid-cols-2 gap-2">
             ...
             <button onClick={()=>{setModalMode('save');setIsMobileMenuOpen(false)}} className="bg-gray-800 hover:bg-gray-700 text-gray-300 py-2 rounded text-[10px] font-bold uppercase tracking-wide border border-gray-700 transition-all">Lưu Game</button>
           </div>
           <button onClick={()=>{setModalMode('load');setIsMobileMenuOpen(false)}} className="w-full bg-gray-800 hover:bg-gray-700 text-gray-300 py-2 rounded text-[10px] font-bold uppercase tracking-wide border border-gray-700 transition-all">Tải Game / Slots</button>
